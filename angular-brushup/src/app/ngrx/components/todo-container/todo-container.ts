...
import { searchTodos } from '../../store/todo-list/todo-list.actions';
...
export class TodoContainer implements OnInit{
  ...
  async ngOnInit() {
    ...
    .subscribe(value => {
      this.store.dispatch(searchTodos({ searchTerm: value || '' }));
    });
  }
  ...
  onSearchChange(searchValue: string) {
    ...
    this.store.dispatch(searchTodos({ searchTerm: searchValue }));
  }
